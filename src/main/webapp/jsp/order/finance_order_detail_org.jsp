...
<script>
	...
	function approve(check,order_no,order_version){
		$.ajax({
			...
			//财务审批回款
			function return_approve(prod_rs_id,prod_flag){
				$.ajax({
					url : 'return_approve.action', //后台处理程序     
					type : 'post', //数据发送方式     
					dataType : 'json', //接受数据格式        
					data : {
					...
					success : function() {
						...
						alert('操作成功');
						location.href="order_Detail.action?order_no="+$('#order_no').val()
   ...
   <div class="modal-dialog">
      <div class="modal-content">
         ...
         <div class="modal-footer" align="center">
            ...
            <button type="button" class="btn btn-success" onclick="approve('pass','${order_list.payment_id}','${order_list.order_version}')">通过审核</button>
					...
					<div class="alert alert-warning top20" role="alert"
						...
						<i class="glyphicon glyphicon-search"></i> 订单基本信息<br>
								...
								<input type="hidden" name="order_no" id="order_no"
									value="${order_list.order_no }"> <input type="hidden"
									name="order_version" id="order_version"
									value="${order_list.order_version }">
								<ul class="list-group">
						...
						<li class="list-group-item"><i class="fa fa-user-secret"></i>&nbsp;
							备注：${order_list.remark }</li>
						<li class="list-group-item"><i class="fa fa-user-secret"></i>&nbsp;
							认购费：${order_list.buy_fee }</li>
						<li class="list-group-item"><i class="fa fa-user-secret"></i>&nbsp;
							开办费：${order_list.start_fee }</li>
										...
										<s:iterator value="#request.lis" var="re">
											<tr class="default">
												...
												<td><input type="number" required="required"
													...
													oninput="chang(${re.prod_rs_id },${re.sale_id })"
													value="${re.return_coe}" /></td>
												<td>${re.sale_id }</td>
												<td>${re.dict_name }</td>
												<td><s:if test="#request.role_id==9">
												<s:if test="#re.prod_flag==1">
														<button
															style=" width: 30%; float: left; text-align: center; padding-left: 0; padding-right: 0;"
															value="" class="btn btn-sm btn-light-info active"
															onclick="return_approve('${re.prod_rs_id }',5)"
															>通过</button>
															<button
															style=" width: 30%; float: left; text-align: center; padding-left: 0; padding-right: 0;"
															value="" class="btn btn-sm btn-light-info active"
															onclick="return_approve('${re.prod_rs_id }',3)"
															>驳回</button>
													</s:if>
													</s:if>
													</td> 
                       ...
                       <li class="list-group-item">
                        ...
                        <input type="text" name="prod_name" style="width: 75%;border:none;outline: none;">
								...
								<li class="list-group-item"><span> 缴款金额&nbsp;:</span> <input
									type="number" required="required" id="pay_amount_play"
									name="pay_amount_play"
						...
						<div class="spjz" style="">
							<ul class="list-group">
								...
								<li class="list-group-item"><span> 打款金额&nbsp:</span> <input
									type="number" required="required" name="pay_amount"
									id="pay_amount"
									style="width: 75%; border: none; outline: none;"></li>
