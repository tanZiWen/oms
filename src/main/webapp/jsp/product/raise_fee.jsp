...
$(function(){
	var r_fee = "<%=request.getAttribute("r_fee")%>";
	$("#raise_fee").val(r_fee);
});

function cancel_raiseFee(){
	parent.location.reload();
}

function raise_fee(){
	var prod_id = $("#prod_id",parent.document).val();
	var raise_fee = $("#raise_fee").val();

	$.ajax({
  	    url:'raise_fee.action',  //后台处理程序     
        type:'post',         //数据发送方式     
		...
		<!-- <form action="add_prod_gp.action" id="form1" name="form1" method="post"> -->
                    <ul class="list-group">
                       ...
                       <li class="list-group-item">
                        ...
                        <span>募集费用&nbsp;:</span>
                        <input id="raise_fee" name="raise_fee" type="text"/>
                       </li>
                    </ul>
          <div class="row text-center">
      <button onclick="cancel_raiseFee()" style="margin-right: 40px;" data-dismiss="modal" class="btn btn-lg btn-default">取消</button>
      <button class="btn btn-lg" data-dismiss="modal" onclick="raise_fee()" style="background-color:#5bc0de; color: #fff;">确定</button>
    </div>
<!--   </form>  -->  
<!--end 右-->
<script>
